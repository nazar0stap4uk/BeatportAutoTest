...
test('should navigate to ukr.net', async ({ page }) => {
  ...
  await frame.locator('input[name="login"]').fill('qweqwe@email');
  await frame.locator('input[name="password"]').fill('qwerty12345');
  ...
  console.log('Текст уведено в обидва поля невірно');
});